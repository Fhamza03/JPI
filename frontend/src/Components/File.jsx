...
export default function File() {
  ...
  const [code, setCode] = useState(fileCode);
  const [revision, setRevision] = useState(rev);
  const [subject, setSubject] = useState(subjectOfRev);
  const [created, setCreated] = useState(created_On);
  const [path, setPath] = useState(pdf_path);
  const [newFileInfos, setNewFileInfos] = useState({});
  const userModifiedFile = sessionStorage.getItem("username");
  ...
  return (
    <div>
      ...
      <div className="card bg-white border border-gray-300 rounded-lg m-4 pt-4 pb-5 pr-2 pl-2 shadow-xl">
        ...
        <div className="flex justify-between mb-2 bg-slate-200 rounded-lg">
          <p className="ml-4 mt-2 mb-2">
            <strong>File Code :</strong> <span> {code}</span>
          </p>
          <p className="mt-2 mb-2">
            <strong>File Name :</strong> <span>{name}</span>
          </p>
          <p className="mr-4 mt-2 mb-2">
            <strong>Revesion :</strong> <span>{revision}</span>
          </p>
        </div>
        <div className="flex justify-between mb-2 rounded-lg">
          <p className="mt-2 mb-2 ml-4">
            <strong>Subject of Revesion :</strong> <span> {subject}</span>
          </p>
          <p className="mt-2 mb-2">
            <strong>Created on :</strong> <span>{formatDate(created)}</span>
          </p>
          <p className="mt-2 mb-2 mr-4">
            <strong>Created by :</strong> <span>Hamza FELLAH</span>
          </p>
        </div>
        <div className="flex justify-between mb-2 bg-slate-200 rounded-lg">
          <p className="ml-4 mt-2 mb-2">
            <strong>Modified By :</strong> <span> {userModifiedFile}</span>
          </p>
          <p className="mr-4 mt-2 mb-2">
            <strong>Modified on :</strong>{" "}
            <span>{`${day}/${month}/${year} at ${hour}:${minute}`}</span>
          </p>
        </div>
      </div>
      <div className="flex justify-end mr-10">
        <button
          onClick={() =>
            ShowPrompt(fileId, name, code, revision, path, subject, created)
          }
